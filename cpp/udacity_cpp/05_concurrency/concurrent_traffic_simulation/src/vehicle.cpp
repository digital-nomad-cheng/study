...
void Vehicle::drive()
{
    ...
    while (true)
    {
        ...
        if (timeSinceLastUpdate >= cycleDuration)
        {
            ...
            // check wether halting position in front of 
            ...
            if (completion >= 1.0 && hasEnteredIntersection)
            {
                ...
                {
                    // pick one street at random and query 
                    // intersection to enter this street
                    std::random_device rd;
                    std::mt19937 eng(rd());
                    std::uniform_int_distribution<> distr(0, 
                                                        streetOptions.size() - 1);
                ...
                // send signal to intersection that vehicle 
